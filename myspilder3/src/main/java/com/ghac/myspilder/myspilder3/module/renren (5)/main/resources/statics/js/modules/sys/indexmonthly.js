$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/indexmonthly/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.indexMonthly.id == null ? "sys/indexmonthly/save" : "sys/indexmonthly/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.indexMonthly),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/indexmonthly/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/indexmonthly/info/"+id, function(r){
                vm.indexMonthly = r.indexMonthly;
            });
		},

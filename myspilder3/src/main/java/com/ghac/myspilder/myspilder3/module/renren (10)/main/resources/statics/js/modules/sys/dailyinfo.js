$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/dailyinfo/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', index: 'id', width: 50, key: true },
			{ label: '', name: 'tradeDate', index: 'trade_date', width: 80 }, 			
			{ label: '', name: 'tsCode', index: 'ts_code', width: 80 }, 			
			{ label: '', name: 'tsName', index: 'ts_name', width: 80 }, 			
			{ label: '', name: 'comCount', index: 'com_count', width: 80 }, 			
			...
			{ label: '', name: 'transCount', index: 'trans_count', width: 80 }, 			
			{ label: '', name: 'pe', index: 'pe', width: 80 }, 			
			{ label: '', name: 'tr', index: 'tr', width: 80 }, 			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.dailyInfo.id == null ? "sys/dailyinfo/save" : "sys/dailyinfo/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.dailyInfo),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/dailyinfo/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/dailyinfo/info/"+id, function(r){
                vm.dailyInfo = r.dailyInfo;
            });
		},

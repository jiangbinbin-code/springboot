$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/margin/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', index: 'id', width: 50, key: true },
			{ label: '', name: 'tradeDate', index: 'trade_date', width: 80 }, 			
			{ label: '', name: 'exchangeId', index: 'exchange_id', width: 80 }, 			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.margin.id == null ? "sys/margin/save" : "sys/margin/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.margin),
                    success: function(r){
                        if(r.code === 0){
		...
		getInfo: function(id){
			$.get(baseURL + "sys/margin/info/"+id, function(r){
                vm.margin = r.margin;
            });
		},

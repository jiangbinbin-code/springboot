$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/indexdaily2/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.indexDaily2.id == null ? "sys/indexdaily2/save" : "sys/indexdaily2/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.indexDaily2),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/indexdaily2/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/indexdaily2/info/"+id, function(r){
                vm.indexDaily2 = r.indexDaily2;
            });
		},

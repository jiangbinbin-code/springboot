$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '', name: 'ratio', index: 'ratio', width: 80 }, 			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.hkHold.id == null ? "sys/hkhold/save" : "sys/hkhold/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.hkHold),
                    success: function(r){
                        if(r.code === 0){
		...
		getInfo: function(id){
			$.get(baseURL + "sys/hkhold/info/"+id, function(r){
                vm.hkHold = r.hkHold;
            });
		},

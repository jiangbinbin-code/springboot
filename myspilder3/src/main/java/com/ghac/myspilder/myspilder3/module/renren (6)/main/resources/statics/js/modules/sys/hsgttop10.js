$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/hsgttop10/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.hsgtTop10.id == null ? "sys/hsgttop10/save" : "sys/hsgttop10/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.hsgtTop10),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/hsgttop10/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/hsgttop10/info/"+id, function(r){
                vm.hsgtTop10 = r.hsgtTop10;
            });
		},

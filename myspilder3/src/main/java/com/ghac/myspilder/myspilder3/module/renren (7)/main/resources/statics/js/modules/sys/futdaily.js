$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '', name: 'change1', index: 'change1', width: 80 }, 			
			{ label: '', name: 'change2', index: 'change2', width: 80 }, 			
			{ label: '', name: 'vol', index: 'vol', width: 80 }, 			
			{ label: '', name: 'amount', index: 'amount', width: 80 }, 			
			{ label: '', name: 'oi', index: 'oi', width: 80 }, 			
			{ label: '', name: 'oiChg', index: 'oi_chg', width: 80 }, 			
			{ label: '', name: 'delvSettle', index: 'delv_settle', width: 80 }			
        ],
		viewrecords: true,
        height: 385,
        rowNum: 10,
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.futDaily.id == null ? "sys/futdaily/save" : "sys/futdaily/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.futDaily),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/futdaily/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/futdaily/info/"+id, function(r){
                vm.futDaily = r.futDaily;
            });
		},

	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.weekly.id == null ? "sys/weekly/save" : "sys/weekly/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.weekly),
                    success: function(r){
                        if(r.code === 0){
		...
		getInfo: function(id){
			$.get(baseURL + "sys/weekly/info/"+id, function(r){
                vm.weekly = r.weekly;
            });
		},
